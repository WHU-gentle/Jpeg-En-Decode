...
/* 统计频率 */
	while (1) {
		data = bitStream.bitstr_getc(stream);
		if (data == EOF) break;
...
void huffman_encode_init_from_codelist(HUFCODEC* phc)
{
...
bool Huffman::huffman_encode_step(HUFCODEC* phc, int data)
{
	...
	if (EOF == bitStream.bitstr_put_bits(phc->output, code, len)) {
		return false;
	}
