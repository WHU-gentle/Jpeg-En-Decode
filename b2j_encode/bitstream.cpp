...
private:
	...
	int fbitstr_putc(int c, void* stream)
	{
		FBITSTR* context = (FBITSTR*)stream;
		if (!context || !context->fp) return EOF;
	...
	int fbitstr_seek(void* stream, long offset, int origin)
	{
		FBITSTR* context = (FBITSTR*)stream;
		if (!context || !context->fp) return EOF;
		context->bitbuf = 0;
	...
	memory bit str
	*/
	void* mbitstr_open(void* buf, int len)
	{
		MBITSTR* context = (MBITSTR*)calloc(1, sizeof(MBITSTR)); // 分配空间
		if (!context) return NULL;
		context->type = BITSTR_MEM;
		context->membuf = (BYTE*)buf;
		context->memlen = len;
		return context;
	}

	int mbitstr_close(void* stream)
	{
	...
	int mbitstr_putc(int c, void* stream)
	{
		MBITSTR* context = (MBITSTR*)stream;
		if (!context || context->curpos >= context->memlen) return EOF;
	...
	int mbitstr_seek(void* stream, long offset, int origin)
	{
		MBITSTR* context = (MBITSTR*)stream;
		int newpos = 0;
		if (!context) return EOF;
